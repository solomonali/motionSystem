...
/////
	...
	float max_xgy[n_S],max_zgy[n_S],holder[n_S];
	...
	for(k=0;k<n_S;k++)
	{
		if( S_imax[k]>250){ offset = 250;}
	        ...
	        start =abs((int)S_imax[k]-offset); //shift from pick to valley
	...
	//	mean_xgy[k] = calculate_mean(x_gy,start,end);
		calculate_Max_Min_Range(z_gy,start,end,(max_zgy+k),(holder+k),
			      (holder+k));

...
*/
        /*x_ac*/
	float mean_xac[n_S],min_xac[n_S],range_xac[n_S],variance1_xac[n_S],
	      variance4_xac[n_S],std_xac[n_S],skewness_xac[n_S],
	      kurtosis_xac[n_S];
//	float *features_xac[] = {mean_xac,max_xac,min_xac,range_xac,MAD_xac,variance_xac, std_xac,skewness_xac,kurtosis_xac};	
//	featureExtraction(n_S,S_imax,features_xac,x_ac);
	...
	for(k=0;k<n_S;k++)
	{
		...
		else if( S_imax[k]>200){ offset = 100;}
		else {offset = 100;} 
		printf("Indx: %f\n",S_imax[k]);
		...
		mean_xac[k]= xftrs[k][0][0];         //mean subseg 1
		min_xac[k] = xftrs[k][2][0];         //min  subseg 1
		range_xac[k]= xftrs[k][3][0];        //range subseg 1
		variance1_xac[k]=xftrs[k][5][0];     //var   subseg 1
		variance4_xac[k]=xftrs[k][5][3];     //var   subseg 4
		std_xac[k] = xftrs[k][6][3];         //std   subseg 4
		skewness_xac[k]= xftrs[k][7][3];     //skewness subseg 4
		kurtosis_xac[k]=xftrs[k][8][3];      //kurtosis subseg 4

	}

//	training_file2(fp,train_file_name,n_S,S_imax,activityCode,t, nft,subSeg,
//			xftrs);

...
int main()
{
	...
	float pk_threshold = 80.00;	// pk-threshold value
	...
	nOutputs = 2;
  	float max;
    	...
    	struct fann *ann;
    	ann = fann_create_from_file("./stage2/TEST.net");
/////
	int ii;
	int start,end;
	int j,k;
	...
	float mean_zgy[BUFF_SIZE],kurtosis_zgy[BUFF_SIZE],holder[BUFF_SIZE],
	      mean_yac[BUFF_SIZE],ratio_yac[BUFF_SIZE],max_yac[BUFF_SIZE],
	      min_yac[BUFF_SIZE],range_yac[BUFF_SIZE],variance_yac[BUFF_SIZE],
	      skewness_yac[BUFF_SIZE],skewness_zgy[BUFF_SIZE];
	float mean_yac3[BUFF_SIZE],variance_xac1[BUFF_SIZE];
	int subIndx = 0;
	int subSeg =4;
	int nft = 9;
	...
	for(k=0;k<n_S;k++)
	{
		...
		mean_zgy[k] = calculate_mean(x_ac,start,end);
	}
	for(k=0;k<n_S;k++)
	{
		if( S_imax[k]>250){ offset = 250;}
	...
	///
		...
		for (i = 0; i < (n_S); i++) {
			...
			{
			    input[j]=(float)(features[j][i]/100);
			    //printf("features: %f\n",features[j][i]/100);
			}	

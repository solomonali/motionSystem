...
void peaks_throughs_file(FILE *fp1, char* ofile_pt_name, int n_P,float *P_i,
		int n_T,float *T_i, double *t,float *axis)
{
	int i, idx;
	FILE *fp;
	const char* pranshu_file = "pt_output.csv";
	/* open the output file to write the peak and trough data */
	printf("Attempting to write to file \'%s\'. Ptr: %p.\n", 
			ofile_pt_name, ofile_pt_name);
	printf("n_P: %d, n_T: %d\n", n_P, n_T);

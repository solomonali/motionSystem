...
void peaks_throughs_file(char* ofile_pt_name, int n_P,float *P_i,
		int n_T,float *T_i, double *t,float *axis);
void stride_file(char* ofile_st_name, int n_S,float *S_imax,
		float *S_imin, double *t,float *axis);
void training_file(char* train_file_name, int n_S,float *S_imax,
...
void training_file2(char* train_file_name, int n_S,float *S_imax,
...
int main(int argc, char **argv)
{
	...
	while ((read = getline(&line, &len, fp)) != -1) {
		/* parse the data */
		rv = sscanf(line, "%lf,%lf,%f,%f,%f,%f,%f,%f\n", &t_b[i], &t_a[i], &x_ac[i], &y_ac[i],
				&z_ac[i],&x_gy[i], &y_gy[i], &z_gy[i]);
		if (rv != 8) {
			fprintf(stderr,
					"%s %d \'%s\'. %s.\n",
					"Failed to read line",
	...
	peaks_throughs_file(ofile_pt_name,n_P,P_i,n_T,T_i,t,z_gy);
       	printf("Hello World! 3\n");	
	stride_file(ofile_st_name,n_S,S_imax,S_imin,t,z_gy);
       	printf("Hello World! 4\n");	
	char * train_file_name = "train_set.txt";
	char * analysis_file_name = "analysis.csv";
	int start,end;
	int j,k;
	float offset=0;
	float holder[n_S];
	...
	training_file2(analysis_file_name,n_S,S_imax,activityCode,t, nft,
		subSeg,	xftrs);

	float mean_yac[n_S],max_yac[n_S],min_yac[n_S],variance_yac[n_S],
	      skewness_yac[n_S],ratio_yac[n_S],
	      mean_yac3[n_S],min_yac1[n_S],range_yac4[n_S],kurtosis_yac2[n_S],
	      variance_yac4[n_S];
...
//		subSeg,	xftrs);
	...
	float mean_zgy[n_S],max_zgy[n_S],min_zgy[n_S],std_zgy[n_S],
	...
	for(k=0;k<n_S;k++)
	{
		...
		mean_zgy[k] = calculate_mean(z_gy,start,end);
 	      	calculate_Max_Min_Range(z_ac,start,end,(max_zgy+k),(min_zgy+k),
				(holder+k));
       		calculate_Statistics (z_gy,start,end,mean_zgy[k],(holder+k),
	         (holder+k),(std_zgy+k),(skewness_zgy+k),(kurtosis_zgy+k));
	}
...
//		subSeg,	xftrs);
	...
	float *features[]={ratio_yac,max_yac,skewness_yac,skewness_zgy};
...
//	fprintf(fp,"start\n");
//	
//	float meanThreshold[] = {1.5,1.5,1.5,0.9};
	int en = 0;
	for (i = 0; i < (n_S); i++) {
...
/*		for(j=0;j<nfeatures;j++)
		{
			if(Features[j][i]>meanFeatures[j]+meanThreshold[j]){ 
				en = 1;}
		       else if(Features[j][i]<meanFeatures[j]-meanThreshold[j]){
				en = 1;}
		}
		if(en==1){en = 0; }
		else{

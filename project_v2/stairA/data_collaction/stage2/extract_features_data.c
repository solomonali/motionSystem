...
/////
	...
	float mean_zgy[n_S],skewness_zgy[n_S],mean_yac[n_S],skewness_yac[n_S],
	      max_yac[n_S],min_yac[n_S],ratio_yac[n_S],holder[n_S];
	float *features[]={skewness_yac,ratio_yac,skewness_zgy};
	while(1)
	{
	...
	for(k=0;k<n_S;k++)
	{
		...
		mean_zgy[k] = calculate_mean(z_gy,start,end);
			...
			(holder+k),(holder+k),(skewness_zgy+k),(holder+k));
